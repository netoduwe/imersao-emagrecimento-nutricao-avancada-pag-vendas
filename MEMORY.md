...
- **Hero Footer (Segunda Dobra):** Otimizado para mobile. O item "CERTIFICADO" foi ocultado especificamente no celular para reduzir a rolagem vertical, e o espaçamento (gap) entre os itens foi reduzido.
- **Correção TopBar:** Removida a duplicação do texto "O evento começa em:" no banner de urgência.
- **PriceCard Fix:** Ajustado o layout do contador de ingressos e preço para evitar compressão em telas de celulares pequenos.
